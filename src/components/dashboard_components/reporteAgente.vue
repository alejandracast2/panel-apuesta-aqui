<template>
  <div>
    <main :class="view_type" id="agentesReportMain">
          ...
          <div class="col-md-12 p-0">
            <div class="row mt-4 mb-3">
              <div class="col-6 col-lg-9 pt-2 pl-4">
                <h5 class="white-text font-weight-bold">
                  Reporte Global de
                  <span style="color: #00bcd4;">{{ targetUser.name }}</span>
                </h5>
              </div>
              <div class="col-6 d-lg-none text-center">
                <button class="d-lg-none btn btn-sm btn-danger waves-effect waves-light"
                  @click="openModal('affiliates_tree_modal')" id="ShowAffiliatesModal">Estructura</button>
              </div>
            </div>
            <div class="row">
                    ...
                    <div class="row col-12 col-xl-11 mx-auto px-0 px-md-2 px-lg-0 mb-2">
                      <div class="col-9 col-sm-5 col-xl-4 mx-auto order-sm-1">
                        <section class="section-preview">
                          <div class="select-wrapper mdb-select col-12">
                            ...
                            <input type="text" id="datesInput" class="select-dropdown w-100 active"
                              @click="showDateOptions" readonly="true" value="" v-model="selectedDate">
                            <ul id="selectDates" class="dropdown-content select-dropdown w-100 text-center active"
                      ...
                      <div class="col-6 col-sm-3 px-sm-2 px-lg-0 px-xl-2 mx-auto custom-date order-sm-4">
                        <div class="md-form">
                          ...
                          <customCalendar class="form-control" :reference="1" :date="startDate" id="start_date"
                            @updateData="event => startDate = event" style="border-color: transparent;" />
                      ...
                      <div class="col-6 col-sm-3 px-sm-2 px-lg-0 px-xl-2 mx-auto custom-date order-sm-6">
                        <div class="md-form">
                          ...
                          <customCalendar class="form-control" :reference="2" :date="endDate" id="start_date"
                            @updateData="event => endDate = event" style="border-color: transparent;" />
                      ...
                      <div class="col-6 col-sm-3 mx-auto text-center order-sm-3">
                        <button class="btn btn-sm btn-cyan waves-effect waves-light" id="change_view"
                          @click="detalles = !detalles">detalles</button>
                      </div>
                    </div>
                    <div id="history_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                      <div class="top"></div>
                      <div v-if="loadingReport" id="users_processing" class="dataTables_processing card">Procesando...
                      </div>
                      <table id="history" class="table table-hover w-100 dataTable no-footer" cellspacing="0"
                        role="grid" style="width: 1247px;">
                        <thead>
                          <tr role="row">
                            <th scope="col" class="sorting_disabled text-center" rowspan="1" colspan="1"
                              style="width: 294px;">Categoria</th>
                            <th scope="col" class="sorting_disabled  d-md-block text-center d-none d-xl-block"
                              rowspan="1" colspan="1" style="width: 165px;">Apuestas</th>
                            <th scope="col" class="sorting_disabled  d-sm-block text-center d-none d-xl-block"
                              rowspan="1" colspan="1" style="width: 165px;">Apostado</th>
                            <th scope="col" class="sorting_disabled  d-sm-block text-center d-none d-xl-block"
                              rowspan="1" colspan="1" style="width: 165px;">Ganado</th>
                            <th scope="col" class="sorting_disabled text-center" rowspan="1" colspan="1"
                        ...
                        <tbody>
                          <tr role="row" class="odd row-no-toggle">
                            ...
                            <td class="d-none d-xl-block d-md-block text-right category3">{{
                              parseNumbers(reportData.casino?.total_bets??0) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category3">{{
                              parseNumbers(reportData.casino?.bet_amount??0) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category3">{{
                              parseNumbers(reportData.casino?.win_amount??0) }}</td>
                            <td class="text-right dt-balance text-nowrap category3">{{
                              parseNumbers(reportData.casino?.netwin??0) }}</td>
                          ...
                          <template v-for="(provider, index) in reportData.casino.providers">
                            <tr role="row" class="even  row-toggle" :class="detalles ? '' : 'd-none'" :key="index">
                              ...
                              <td class="d-none d-xl-block d-md-block text-right"
                                style="background-color: rgb(230, 230, 250);">{{ parseNumbers(provider.total_bets) }}
                              </td>
                              <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                                style="background-color: rgb(230, 230, 250);">{{ parseNumbers(provider.bet_amount) }}
                              </td>
                              <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                                style="background-color: rgb(230, 230, 250);">{{ parseNumbers(provider.win_amount) }}
                              </td>
                              <td class="text-right dt-balance text-nowrap"
                                style="background-color: rgb(230, 230, 250);">{{ parseNumbers(provider.netwin) }}</td>
                              <td class="text-right" style="background-color: rgb(230, 230, 250);">{{
                                parseNumbers(provider.comision) }}</td>
                            </tr>
                            <tr role="row" class="odd  row-toggle" :class="detalles ? '' : 'd-none'"
                              v-for="(brand, bindex) in provider.brands" :key="bindex + '-' + brand.provider">
                              <td class="text-left dt-category" style="background-color: rgb(250, 250, 250);"><span
                                  class="ml-1 ml-sm-3"></span>{{ bindex }}</td>
                              <td class="d-none d-xl-block d-md-block text-right"
                                style="background-color: rgb(250, 250, 250);">{{ parseNumbers(brand.total_bets) }}</td>
                              <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                                style="background-color: rgb(250, 250, 250);">{{ parseNumbers(brand.bet_amount) }}</td>
                              <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                                style="background-color: rgb(250, 250, 250);">{{ parseNumbers(brand.win_amount) }}</td>
                              <td class="text-right dt-balance text-nowrap"
                                style="background-color: rgb(250, 250, 250);">{{ parseNumbers(brand.netwin) }}</td>
                              <td class="text-right" style="background-color: rgb(250, 250, 250);">{{
                                parseNumbers(brand.comision) }}</td>
                            </tr>
                          </template>
                          <tr role="row" class="odd row-no-toggle">
                            <td class="text-left dt-category category2"><span class="ml-0 ml-sm-1"></span> Deportes</td>
                            <td class="d-none d-xl-block d-md-block text-right category2">{{
                              parseNumbers(reportData.deportes?.total_bets??0) }}
                            </td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category2">{{
                              parseNumbers(reportData.deportes?.bet_amount??0) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category2">{{
                              parseNumbers(reportData.deportes?.win_amount??0) }}</td>
                            <td class="text-right dt-balance text-nowrap category2">{{
                              parseNumbers(reportData.deportes?.netwin??0) }}</td>
                          ...
                          <tr role="row" class="even  row-toggle" :class="detalles ? '' : 'd-none'"
                            v-if="Object.keys(reportData.deportes).length > 0">
                            <td class="text-left dt-category" style="background-color: rgb(250, 250, 250);"><span
                                class="ml-1 ml-sm-3"></span>
                              Deportes</td>
                            <td class="d-none d-xl-block d-md-block text-right"
                              style="background-color: rgb(250, 250, 250);">{{
                                parseNumbers(reportData.deportes?.total_bets) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                              style="background-color: rgb(250, 250, 250);">{{
                                parseNumbers(reportData.deportes?.bet_amount) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                              style="background-color: rgb(250, 250, 250);">{{
                                parseNumbers(reportData.deportes?.win_amount) }}</td>
                            <td class="text-right dt-balance text-nowrap" style="background-color: rgb(250, 250, 250);">
                              {{
                                parseNumbers(reportData.deportes?.netwin) }}</td>
                            <td class="text-right" style="background-color: rgb(250, 250, 250);">{{
                              parseNumbers(reportData.deportes?.comision) }}
                            </td>
                          </tr>
                          <tr role="row" class="odd row-no-toggle">
                            <td class="text-left dt-category category1"><span class="ml-0 ml-sm-1"></span> Poker</td>
                            <td class="d-none d-xl-block d-md-block text-right category1">{{
                              parseNumbers(reportData.poker?.total_bets??0) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category1">{{
                              parseNumbers(reportData.poker?.bet_amount??0) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category1">{{
                              parseNumbers(reportData.poker?.win_amount??0) }}</td>
                            <td class="text-right dt-balance text-nowrap category1">{{
                              parseNumbers(reportData.poker?.netwin??0) }}</td>
                            <td class="text-right category1"
                              @mouseenter="showPopover('#rake', '#popoverdata', parseNumbers(reportData.poker?.rake ?? 0))">
                              <div data-toggle="popover" data-placement="top" title="" data-html="true"
                                @mouseout="showPopover2 = false" data-content="Rake: 9.291,94" data-original-title="">
                                <span id="rake">{{
                                  parseNumbers(reportData.poker?.comision??0) }}</span></div>
                            </td>
                          </tr>
                          <!-- DETALLES DE POKER -->
                          <tr role="row" class="even  row-toggle" :class="detalles ? '' : 'd-none'"
                            v-if="Object.keys(reportData.poker).length > 0">
                            <td class="text-left dt-category" style="background-color: rgb(250, 250, 250);"><span
                                class="ml-1 ml-sm-3"></span>
                              Poker</td>
                            <td class="d-none d-xl-block d-md-block text-right"
                              style="background-color: rgb(250, 250, 250);">{{
                                parseNumbers(reportData.poker?.total_bets) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                              style="background-color: rgb(250, 250, 250);">{{
                                parseNumbers(reportData.poker?.bet_amount) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"
                              style="background-color: rgb(250, 250, 250);">{{
                                parseNumbers(reportData.poker?.win_amount) }}</td>
                            <td class="text-right dt-balance text-nowrap" style="background-color: rgb(250, 250, 250);">
                              {{
                                parseNumbers(reportData.poker?.netwin) }}</td>
                            <td class="text-right" style="background-color: rgb(250, 250, 250);">
                              <div data-toggle="popover" data-placement="top" title="" data-html="true"
                                data-content="Rake: 9.291,94" data-original-title="">{{
                                  parseNumbers(reportData.poker?.comision) }}</div>
                            </td>
                          ...
                          <tr role="row" class="odd row-no-toggle">
                            ...
                            <td class="d-none d-xl-block d-md-block text-right category5">{{
                              parseNumbers(reportData.bingo?.total_bets??0) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category5">{{
                              parseNumbers(reportData.bingo?.bet_amount??0) }}</td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right category5">{{
                              parseNumbers(reportData.bingo?.win_amount??0) }}</td>
                            <td class="text-right dt-balance text-nowrap category5">{{
                              parseNumbers(reportData.bingo?.netwin??0) }}</td>
                          ...
                          <tr role="row" class="odd  row-toggle" :class="detalles ? '' : 'd-none'">
                            ...
                            <td class="text-right" style="font-style: italic;">{{ parseNumbers(reportData.totales.casino
                              -
                              reportData.casino.comision) }}</td>
                          ...
                          <tr role="row" class="even  row-toggle" :class="detalles ? '' : 'd-none'">
                            ...
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"></td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"></td>
                            <td class="text-right dt-balance text-nowrap" style="font-style: italic;">Total Deportes
                            </td>
                            <td class="text-right" style="font-style: italic;">{{
                              parseNumbers(reportData.totales.deportes -
                              reportData.deportes.comision) }}</td>
                          ...
                          <tr role="row" class="odd  row-toggle" :class="detalles ? '' : 'd-none'">
                            ...
                            <td class="text-right" style="font-style: italic;">{{
                              parseNumbers((reportData.totales?.poker ?? 0) -
                              (reportData.poker?.comision??0)) }}</td>
                          ...
                          <tr role="row" class="even  row-toggle" :class="detalles ? '' : 'd-none'">
                            ...
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"></td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"></td>
                            <td class="text-right dt-balance text-nowrap" style="font-style: italic;">Total Bingo en
                              vivo
                            </td>
                            <td class="text-right" style="font-style: italic;">{{
                              parseNumbers((reportData.totales?.bingo ?? 0) -
                              (reportData.bingo?.comision??0)) }}</td>
                          ...
                          <tr role="row" class="even row-no-toggle" style="line-height: 30px;">
                            ...
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"></td>
                            <td class="text-right dt-balance text-nowrap"
                              style="font-weight: bold; background-color: rgb(242, 242, 242);">Total a
                              pagar:</td>
                          ...
                          <tr role="row" class="odd  row-toggle" style="line-height: 1.5;"
                          ...
                          <tr role="row" class="even  row-toggle" style="background-color: rgb(242, 242, 242);"
                            :class="detalles ? '' : 'd-none'">
                            ...
                            <td class="text-right dt-balance text-nowrap">{{ parseNumbers(reportData.fichas.total) }}
                            </td>
                            <td class="text-right"></td>


                          </tr>
                          <tr role="row" class="odd  row-toggle" style="line-height: 30px;"
                            :class="detalles ? '' : 'd-none'">
                            <td class="text-left dt-category"></td>
                            <td class="d-none d-xl-block d-md-block text-right"></td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"></td>
                            <td class="d-none d-sm-block d-xl-block d-md-block text-right"></td>
                            <td class="text-right dt-balance text-nowrap"
                              style="font-weight: bold; background-color: rgb(242, 242, 242);">Total
                              por ventas:</td>
                            <td class="text-right" style="font-weight: bold; background-color: rgb(242, 242, 242);">
                              {{ parseNumbers(reportData.fichas.total - (reportData.casino.comision ?? 0 -
                                reportData.poker.comision ?? 0 -
                                reportData.bingo.comision??0) - (reportData.deportes?.comision??0)) }}</td>
                ...
                <div class="tree card py-3 px-0 jstree-default" id="affiliates_tree" role="tree">
                  ...
                  <ul class="px-1 py-0" style="font-size: small;">
                    <treeNode :node="main_tree" :startDate="getFechaFormated(startDate)"
                      :endDate="getFechaFormated(endDate)" :startHour="startHour" :endHour="endHour" />
                  </ul>
                </div>
              </div>
            </div>
      ...
      <div class="p-1" role="tooltip" id="popoverdata" style="z-index: 7;background-color: white;font-size:small;"
        :style="showPopover2 ? 'display:block' : 'display:none'">
        Rake: {{ popoverData }}
        <div id="arrow" data-popper-arrow></div>
      </div>
      <div class="modal fade left" id="affiliates_tree_modal" tabindex="-1" role="dialog" aria-labelledby="affTreeModal"
        aria-hidden="true">
        <div class="modal-dialog modal-full-height modal-left" role="document">
          <div class="modal-content">
            ...
            <div style="max-height:65vh;overflow-y:scroll;overflow-x:hidden" class="tree py-3 px-0 jstree-default"
              id="affiliates_tree" role="tree">
              <div class="switch font-size-custom py-1"><label>Incluir ocultos<input id="TreeShowHidden" type="checkbox"
                    @change="showHiddenTree"><span class="lever"></span></label>
              </div>
              <ul class="px-1 py-0" style="font-size: small;">
                <treeNode :node="main_tree" :startDate="getFechaFormated(startDate)"
                  :endDate="getFechaFormated(endDate)" :startHour="startHour" :endHour="endHour" />
